...
class PIDController:
    def __init__(self, z_des=0.1, rpy_setpoint=[0, 0, 0], state_estimator=None):
        ...
        self.pid_roll = PID(
            6, 0.5, 1.25, setpoint=rpy_setpoint[0], output_limits=(-1, 1)
        )
        self.pid_pitch = PID(
            6, 0.5, 1.25, setpoint=rpy_setpoint[1], output_limits=(-1, 1)
        )
    ...
    def compute_control(self):
        ...
        thrust_total = (
            (self.pid_alt(self.state_estimator.alt) + 9.81)
            * self.state_estimator.mass
            / (np.cos(self.state_estimator.roll) * np.cos(self.state_estimator.pitch))
        )
