...
class Drone:
    ...
    def __call__(self):
        ...
        f, M = self.se3_controller.tracking_control(
            pos_des=[0.5, 0.5, 0.5], heading_des=[1, 0, 0]
        )
    ...
    def cal_motor_cmd(self, T, tau_x, tau_y, tau_z):
        dx, dy, k = (
            ...
            0.18,
            0.1,
        )  # distance from motor to x axis, y aixs, motor constant
        gain = 4
...
def main():
    ...
    with mujoco.viewer.launch_passive(drone.m, drone.d) as viewer:
        ...
        viewer.opt.flags[mujoco.mjtVisFlag.mjVIS_ACTUATOR] = (
            True  # Enable site visualization
        )
